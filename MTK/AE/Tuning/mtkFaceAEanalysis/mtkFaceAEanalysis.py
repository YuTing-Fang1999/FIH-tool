...
def faceDetect(img_path):
    ...
    # Load the cascade
    face_cascade = cv2.CascadeClassifier('MTK/AE/Tuning/mtkFaceAEanalysis/haarcascade_frontalface_alt2.xml')
...
if __name__ == "__main__":
    ...
    exif_path = "MTK/AE/Tuning/mtkFaceAEanalysis/Exif"
...
class GenExcelWorkerThread(QThread):
        ...
        def run(self):
            ...
            allFileList = os.listdir(self.exif_path)
            allFileList_exif = np.sort(allFileList,axis=0)
            allFileList_exif = list(filter(file_filter, allFileList_exif))
            allFileList_exif.sort(key=natural_keys)
            allFileList_jpg = np.sort(allFileList,axis=0)
            ...
            for i in range(0,(np.size(allFileList_exif))):
                path_name = self.exif_path + "/" + allFileList_exif[i]
                exifFile = open(path_name, "r")
                file_name = os.path.basename(path_name)
                base = os.path.splitext(file_name)[0]
                baseTag = base.split(".")[0]
                ...
                if i == 0:
                    startNum = re.sub("[^0-9-,]","", base[0:2])
                    wb = self.create_xls(self.code_path, self.base_excel_path)
                    wb.active = 0
                    ws = wb.active
                
                print(base)
                self.update_status_signal.emit(base)
                ...
                for j in range(0,(np.size(allFileList_jpg))):
                    path_name_jpg = self.exif_path + "/" + allFileList_jpg[j]
                    file_name_jpg = os.path.basename(path_name_jpg)
                    base2 = os.path.splitext(file_name_jpg)[0]
                    ...
                    if file_name_jpg == base or base2 == base[0:-8]:
                        ...
                        save_name = self.exif_path + "/" + os.path.splitext(file_name_jpg)[0] + "_crop.png"
                        img_crop = faceDetect(path_name_jpg)
                        if img_crop is None:                
                            print("圖片讀取失敗！")
                        # 儲存含中文檔名的圖片
