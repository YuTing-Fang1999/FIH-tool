...
def classify(BVnum,B2D,midratio,B2D_region,midratio_region, exif_path):
    Path(exif_path+"/"+BVnum).mkdir(parents=True, exist_ok=True)
    if B2D < B2D_region[0]:
        Path(exif_path+"/"+BVnum+f"/B2D1_{B2D_region[0]}down").mkdir(parents=True, exist_ok=True)
        if midratio < midratio_region[0]:
            final_path = exif_path+"/"+BVnum+f"/B2D1_{B2D_region[0]}down/midratio1_{midratio_region[0]}down"
            Path(final_path).mkdir(parents=True, exist_ok=True)
        for nummidratio in range(0,np.size(midratio_region)-1,1):
            if midratio >= midratio_region[nummidratio] and midratio < midratio_region[nummidratio+1]:
                final_path = exif_path+"/"+BVnum+f"/B2D1_{B2D_region[0]}down/midratio{nummidratio+2}_{midratio_region[nummidratio]}_{midratio_region[nummidratio+1]}"
                Path(final_path).mkdir(parents=True, exist_ok=True)
        if midratio > midratio_region[np.size(midratio_region)-1]:
            final_path = exif_path+"/"+BVnum+f"/B2D1_{B2D_region[0]}down/midratio{np.size(midratio_region)+1}_{midratio_region[np.size(midratio_region)-1]}up"
            Path(final_path).mkdir(parents=True, exist_ok=True)
    for numB2D in range(0,np.size(B2D_region)-1,1):
        if B2D >= B2D_region[numB2D] and B2D < B2D_region[numB2D+1]:
            Path(exif_path+"/"+BVnum+f"/B2D{numB2D+2}_{B2D_region[numB2D]}_{B2D_region[numB2D+1]}").mkdir(parents=True, exist_ok=True)
            if midratio < midratio_region[0]:
                final_path = exif_path+"/"+BVnum+f"/B2D{numB2D+2}_{B2D_region[numB2D]}_{B2D_region[numB2D+1]}/midratio1_{midratio_region[0]}down"
                Path(final_path).mkdir(parents=True, exist_ok=True)
            for nummidratio in range(0,np.size(midratio_region)-1,1):
                if midratio >= midratio_region[nummidratio] and midratio < midratio_region[nummidratio+1]:
                    final_path = exif_path+"/"+BVnum+f"/B2D{numB2D+2}_{B2D_region[numB2D]}_{B2D_region[numB2D+1]}/midratio{nummidratio+2}_{midratio_region[nummidratio]}_{midratio_region[nummidratio+1]}"
                    Path(final_path).mkdir(parents=True, exist_ok=True)
            if midratio > midratio_region[np.size(midratio_region)-1]:
                final_path = exif_path+"/"+BVnum+f"/B2D{numB2D+2}_{B2D_region[numB2D]}_{B2D_region[numB2D+1]}/midratio{np.size(midratio_region)+1}_{midratio_region[np.size(midratio_region)-1]}up"
                Path(final_path).mkdir(parents=True, exist_ok=True)
    if B2D > B2D_region[np.size(B2D_region)-1]:
        Path(exif_path+"/"+BVnum+f"/B2D{np.size(B2D_region)+1}_{B2D_region[np.size(B2D_region)-1]}up").mkdir(parents=True, exist_ok=True)
        if midratio < midratio_region[0]:
            final_path = exif_path+"/"+BVnum+f"/B2D{np.size(B2D_region)+1}_{B2D_region[np.size(B2D_region)-1]}up/midratio1_{midratio_region[0]}down"
            Path(final_path).mkdir(parents=True, exist_ok=True)
        for nummidratio in range(0,np.size(midratio_region)-1,1):
            if midratio >= midratio_region[nummidratio] and midratio < midratio_region[nummidratio+1]:
                final_path = exif_path+"/"+BVnum+f"/B2D{np.size(B2D_region)+1}_{B2D_region[np.size(B2D_region)-1]}up/midratio{nummidratio+2}_{midratio_region[nummidratio]}_{midratio_region[nummidratio+1]}"
                Path(final_path).mkdir(parents=True, exist_ok=True)
        if midratio > midratio_region[np.size(midratio_region)-1]:
            final_path = exif_path+"/"+BVnum+f"/B2D{np.size(B2D_region)+1}_{B2D_region[np.size(B2D_region)-1]}up/midratio{np.size(midratio_region)+1}_{midratio_region[np.size(midratio_region)-1]}up"
            Path(final_path).mkdir(parents=True, exist_ok=True)
    return final_path
        ...
        with open(path_name, 'r' ) as exifFile :
            ...
            for line in exifFile:
                ...
                if "AE_TAG_DRV6_B2D" in line:
                    B2D = int(re.sub("[^0-9-,]","", line)[2:])
                if "AE_TAG_DRV6_CORR_MIDRATIO" in line:
                    midratio = int(re.sub("[^0-9-,]","", line)[1:])
            ...
            for j in range(0,(np.size(allFileList_jpg))):
                ...
                if file_name_jpg == base:
                    ...
                    BV_all.append([base.split("_")[0],BV,B2D,midratio])
                    ...
                    if faceProb[0] != 0 or faceProb[4] != 0:
                        ...
                        if BV < BV_region[0]:
                            locals()['BV1'].append([base.split("_")[0],BV,B2D,midratio])
                            destination = classify(f"BV1_{BV_region[0]}down",B2D,midratio,B2D_region,midratio_region, exif_path)
                        for numBV in range(0,np.size(BV_region)-1,1):
                            if BV >= BV_region[numBV] and BV < BV_region[numBV+1]:
                                locals()['BV'+str(numBV+2)].append([base.split("_")[0],BV,B2D,midratio])
                                destination = classify(f"BV{numBV+2}_{BV_region[numBV]}_{BV_region[numBV+1]}",B2D,midratio,B2D_region,midratio_region, exif_path)
