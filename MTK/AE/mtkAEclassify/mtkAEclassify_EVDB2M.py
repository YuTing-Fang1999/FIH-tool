...
def EVDB2M (exif_path):
    ...
    for level1 in data:
        ...
        for level2 in data[level1]:
            ...
            plot_and_save(EVD_list, f"{level1}_{level2}", f"/{level1}/{level2}/{level1}_{level2}.png", EVDthd, B2MThd, exif_path, xlim, [0,max_y])
            BV_list += EVD_list
        
        plot_and_save(BV_list, f"{level1}", f"/{level1}/{level1}.png", EVDthd, B2MThd, exif_path, [0,max_x], [0,max_y])
