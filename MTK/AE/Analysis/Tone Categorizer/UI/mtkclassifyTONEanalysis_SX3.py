...
def classify(LVnum, LV, DR, DR_region, yourPath):
    ...
    for numDR in range(0, np.size(DR_region)-1, 1):
        if DR >= DR_region[numDR] and DR < DR_region[numDR+1]:
            final_path = yourPath+"/"+LVnum + \
                f"/DR{numDR+2}_{DR_region[numDR]}_{DR_region[numDR+1]}"
            Path(final_path).mkdir(parents=True, exist_ok=True)
    if DR > DR_region[np.size(DR_region)-1]:
        final_path = yourPath+"/"+LVnum + \
            f"/DR{np.size(DR_region)+1}_{DR_region[np.size(DR_region)-1]}up"
        Path(final_path).mkdir(parents=True, exist_ok=True)
    return final_path
...
def create_xls(file_path):
    ...
    for line in f:
        ...
        if "Bright    Strength" in line:
            ...
            if brightTBL == []:
                brightTBL.append(
                    re.sub("[^0-9-,]", "", line3).split(",")[0:-1])
                brightTBL.append(
                    re.sub("[^0-9-,]", "", line4).split(",")[0:-1])
                brightTBL.append(
                    re.sub("[^0-9-,]", "", line5).split(",")[0:-1])
                brightTBL.append(
                    re.sub("[^0-9-,]", "", line6).split(",")[0:-1])
                brightTBL.append(
                    re.sub("[^0-9-,]", "", line7).split(",")[0:-1])
                brightTBL.append(
                    re.sub("[^0-9-,]", "", line8).split(",")[0:-1])
                brightTBL.append(
                    re.sub("[^0-9-,]", "", line9).split(",")[0:-1])
                brightTBL.append(
                    re.sub("[^0-9-,]", "", line10).split(",")[0:-1])
                brightTBL.append(
                    re.sub("[^0-9-,]", "", line11).split(",")[0:-1])
                brightTBL.append(
                    re.sub("[^0-9-,]", "", line12).split(",")[0:-1])
                pre_slash, post_slash = line13.split('//')
    ...
    for j, item in enumerate(darkTBL, start=0):
        for i in range(0, np.size(item)):
            ws.cell(column=18+i, row=7 +
                    j).value = str(item[i] + " / " + brightTBL[j][i])
...
def classify_xls(yourPath, file_path):
    ...
    for i, item in enumerate(allFileList_exif, start=0):
        ...
        for line in exifFile:
            if "SW_LCE_P0" in line:
                ws.cell(column=15, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[1:])
            if "SW_LCE_P1" in line:
                ws.cell(column=16, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[1:])
            if "SW_LCE_P50" in line:
                if checkP50 == 0:
                    ws.cell(column=17, row=23+(5*(i % 20))
                            ).value = int(re.sub("[^0-9-,]", "", line)[2:])
                    checkP50 = 1
            if "SW_LCE_P250" in line:
                ws.cell(column=18, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[3:])
            if "SW_LCE_P500" in line:
                ws.cell(column=19, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[3:])
            if "SW_LCE_P750" in line:
                ws.cell(column=20, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[3:])
            if "SW_LCE_P950" in line:
                ws.cell(column=21, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[3:])
            if "SW_LCE_P999" in line:
                ws.cell(column=22, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[3:])
            if "SW_LCE_O0" in line:
                ws.cell(column=15, row=24+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[1:])
            if "SW_LCE_O1" in line:
                ws.cell(column=16, row=24+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[1:])
            if "SW_LCE_O50" in line:
                if checkO50 == 0:
                    ws.cell(column=17, row=24+(5*(i % 20))
                            ).value = int(re.sub("[^0-9-,]", "", line)[2:])
                    checkO50 = 1
            if "SW_LCE_O250" in line:
                ws.cell(column=18, row=24+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[3:])
            if "SW_LCE_O500" in line:
                ws.cell(column=19, row=24+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[3:])
            if "SW_LCE_O750" in line:
                ws.cell(column=20, row=24+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[3:])
            if "SW_LCE_O950" in line:
                ws.cell(column=21, row=24+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[3:])
            if "SW_LCE_O999" in line:
                ws.cell(column=22, row=24+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line)[3:])

            if "SW_LCE_LumaProb" in line:
                ws.cell(column=24, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line))
            if "SW_LCE_FlatProb" in line:
                ws.cell(column=25, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line))
            if "SW_LCE_LVProb" in line:
                ws.cell(column=44, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line))

            if "SW_LCE_LumaTarget" in line:
                ws.cell(column=28, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line))
            if "SW_LCE_FlatTarget" in line:
                ws.cell(column=29, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line))
            if "SW_LCE_LumaFlatTarget" in line:
                ws.cell(column=30, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line))
            if "SW_LCE_FinalTarget" in line:
                ws.cell(column=32, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line))

            if "SW_LCE_LV" in line:
                if checkLV == 0:
                    ws.cell(column=33, row=23+(5*(i % 20))
                            ).value = int(re.sub("[^0-9-,]", "", line))
                    checkLV = 1
            if "SW_LCE_CurrDR" in line:
                ws.cell(column=36, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line))

            if "SW_LCE_FinalDStrength" in line:
                ws.cell(column=41, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line))
            if "SW_LCE_FinalBStrength" in line:
                ws.cell(column=42, row=23+(5*(i % 20))
                        ).value = int(re.sub("[^0-9-,]", "", line))
...
def main(yourPath, file_path, LV_region, DR_region):
            ...
            if base == item_jpg:
                ...
                if LV < LV_region[0]:
                    ...
                    destination = classify(
                        f"LV1_{LV_region[0]}down", LV, DR, DR_region, yourPath)
                    break
                for numLV in range(0, np.size(LV_region)-1, 1):
                    if LV >= LV_region[numLV] and LV < LV_region[numLV+1]:
                        locals()['LV'+str(numLV+2)
                                 ].append([base.split("_")[0], LV, DR])
                        destination = classify(
                            f"LV{numLV+2}_{LV_region[numLV]}_{LV_region[numLV+1]}", LV, DR, DR_region, yourPath)
                        break
                if LV > LV_region[np.size(LV_region)-1]:
                    locals()['LV'+str(np.size(LV_region)+1)
                             ].append([base.split("_")[0], LV, DR])
                    destination = classify(
                        f"LV{np.size(LV_region)+1}_{LV_region[np.size(LV_region)-1]}up", LV, DR, DR_region, yourPath)
                    break
                exifFile.close()
    ...
    destination_set = [x for i, x in enumerate(
        destination_set) if x not in destination_set[:i]]
