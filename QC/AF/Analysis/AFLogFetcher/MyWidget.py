...
import os
import json
from QC.AF.Analysis.AFLogFetcher import main_UIParser
import threading
import time
...
class MyWidget(ParentWidget):
    ...
    def __init__(self):
        ...
        #global variable
        self.CONFIG_FOLDER = "QC\\AF\\Analysis\\AFLogFetcher\\config\\"
        self.PHOTO_DIR = ""
        self.LOG_DIR = ""
        self.OUTPUT_DIR = ""
        self.PROJECT_NAME = ""
    ...
    def showPhotoFileDialog(self):
        ...
        # Open file dialog
        self.PHOTO_DIR = QFileDialog.getExistingDirectory(self,"選擇圖片資料夾", self.get_path("QC_AF_folder"))
        if self.PHOTO_DIR == "": return
        self.ui.readImagesDirLabel.setText(self.PHOTO_DIR)
        self.set_path('QC_AF_folder', '/'.join(self.PHOTO_DIR.split('/')[:-1]))
        self.detectParserReady()
    ...
    def showLogFileDialog(self):
        ...
        # Open file dialog
        self.LOG_DIR = QFileDialog.getExistingDirectory(self,"選擇log資料夾", self.get_path("QC_AF_folder"))
        if self.LOG_DIR == "": return
        self.ui.readLogDirLabel.setText(self.LOG_DIR)
        self.set_path('QC_AF_folder', '/'.join(self.LOG_DIR.split('/')[:-1]))
        self.detectParserReady()
    ...
    def showOutputFileDialog(self):
        ...
        # Open file dialog
        self.OUTPUT_DIR = QFileDialog.getExistingDirectory(self,"選擇匯出資料夾", '/'.join(self.get_path("QC_AF_output_folder").split('/')[:-1]))
