...
class GraphicItem(QGraphicsRectItem):
    def __init__(self, x1, x2, w, parent=None):
        ...
        self.setPen(Qt.red)
        r = QRectF(x1, x2, w, w*1.5)  # 起始座標,寬,長
        self.setRect(r)
...
class ImageViewer(QtWidgets.QGraphicsView):
    ...
    def __init__(self, parent=None):
        ...
        self.square_rate = 0.03
        self.padding_rate = 0.0203
        self.start_h_rate = 0.02
        self.start_w_rate = 0.006
    ...
    def wheelEvent(self, event):
        if self.is_key_press_ctrl:
            if event.angleDelta().y() > 0:
                self.square_rate += 0.001
                if self.square_rate>0.05:
                    self.square_rate=0.05

            else:
                self.square_rate -= 0.001
                if self.square_rate<0.001:
...
if __name__ == '__main__':
    ...
    filename = "QC/AE/Calibration/stepChart/step_3_before.jpg"
